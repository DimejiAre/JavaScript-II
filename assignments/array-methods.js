...
let participatingCompanies = runners.map((runner)=> runner.company_name);
console.log("participatingCompanies: ", participatingCompanies);

// Problem 2
// Runners who donated more than 250
let heavyDonors = runners.filter((runner)=> runner.donation > 250);
console.log("heavyDonors: ", heavyDonors);

// Problem 3
// list Shirt sizes to know which to get
let shirtSizes = runners.map((runner)=> runner.shirt_size);
let uniqueShirtSizes = shirtSizes.filter((shirtSize, index)=> shirtSizes.indexOf(shirtSize) === index);
console.log("unique shirt sizes: ", uniqueShirtSizes);